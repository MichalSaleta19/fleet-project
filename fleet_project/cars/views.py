...
def car_add_view(request):
    ...
    if request.method == "POST":
        ...
        if not brand or not model or not year or not model_year or not vin_number:
            return render(
                request, template_name, {"error": "Please fill in all required fields"}
            )
...
def driver_delete_view(request, pk):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "confirm_delete.html",
            {"object": driver, "next_url": "driver_list"},
        )
...
def fleet_manager_modify_view(request, pk):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "fleetmanager_modify.html", {"fleet_manager": fleet_manager}
        )
...
def fleet_manager_delete_view(request, pk):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "fleetmanager_confirm_delete.html",
            {"object": fleet_manager, "next_url": "fleetmanager_list"},
        )
...
def order_add_view(request, car_id):
    if request.method == "POST":
        ...
        Reservation.objects.create(
            car_id=car_id, start_date=pickup_date, end_date=return_date
        )
...
def service_history_details_view(request, service_history_id):
    ...
    return render(
        request, "servicehistory_details.html", {"service_history": service_history}
    )
...
def service_history_add_view(request):
    if request.method == "POST":
        ...
        service_history = ServiceHistory(
            car_id=car_id, service_date=service_date, description=description, cost=cost
        )
...
def service_history_modify_view(request, service_history_id):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "servicehistory_modify.html", {"service_history": service_history}
        )
...
def availability_list_view(request):
    ...
    return render(
        request, "availability_list.html", {"availability_list": availability_list}
    )
...
def availability_add_view(request):
    if request.method == "POST":
        ...
        availability = Availability(
            car_id=car_id, start_date=start_date, end_date=end_date, status=status
        )
        availability.save()
...
def availability_modify_view(request, availability_id):
    ...
    if request.method == "POST":
        ...
        return render(
            request, "availability_modify.html", {"availability": availability}
        )
...
def register_view(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            return render(
                request, "register.html", {"error_message": "Username already exists"}
            )
        ...
        if user:
            ...
            return render(
                request, "register.html", {"error_message": "Failed to register user"}
            )
    else:
